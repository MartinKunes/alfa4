        ...
        while True:
            ...
            if 'command' in message and 'peer_id' in message:
                if message['command'] == 'hello' and message['peer_id'] != peer_id:
                    ...
                    addresses.append(addr)  # Add this line to add the address to the list
...
def udp_client():
    try:
        ...
        while True:
            sock.sendto(json.dumps({"command": "hello", "peer_id": peer_id}).encode(), ('172.31.255.255', 9876))
        ...
        while True:
            ...
            if isinstance(message, dict) and 'command' in message:
                if message['command'] == 'hello':
                    ...
                    conn.send(json.dumps({"status": "ok"}).encode())  # Add this line to send 'ok' for all commands
            conn.close()
    except Exception as e:
...
def tcp_client():
    try:
        for addr in addresses:  # Change this line to iterate over addresses
            ...
            try:
                ...
                if isinstance(message, dict):
                    ...
                    print(f"EXCEPTION: tcp_client: {peer_id}: expected dictionary, got {type(message)}")
                sock.close()
            except ConnectionError:
                print(f"EXCEPTION: tcp_client: {peer_id}: connection closed")
    except Exception as e:
